...
export async function buildLlmsTxtMarkdown(): Promise<string> {
  ...
  return [
    ...
    mdLink(
      "Post a job need in Nanganallur",
      "/local-job-request-nanganallur",
      "Seekers post one public request; employers contact them on WhatsApp.",
    ),
...
export async function buildLlmsFullTxtMarkdown(): Promise<string> {
  ...
  return [
    ...
    `- Post a job need (Nanganallur): ${absoluteUrl("/local-job-request-nanganallur")}`,
