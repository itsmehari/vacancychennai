...
import { requestAdminPasswordResetAction } from "@/features/auth/account-actions";
import { loginQueryErrorMessage, loginQueryInfoMessage } from "@/lib/auth-login-errors";
import { hasDatabase } from "@/lib/db";

export const metadata: Metadata = {
  title: "Admin forgot password | Vacancy Chennai",
  description: "Reset your Vacancy Chennai admin password via email.",
    ...
    <AuthPageShell>
      <section className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Enter the email on your admin account. If it matches, we’ll send a one-time link (valid about 1
          hour).
        </p>
