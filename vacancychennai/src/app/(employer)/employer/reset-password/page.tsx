...
import { resetEmployerPasswordAction } from "@/features/auth/account-actions";
import { loginQueryErrorMessage } from "@/lib/auth-login-errors";
import { hasDatabase } from "@/lib/db";
import { validatePasswordResetToken } from "@/lib/email/verification-tokens";

export const metadata: Metadata = {
  title: "Choose new password | Vacancy Chennai",
  description: "Set a new password for your employer account.",
...
export default async function EmployerResetPasswordPage({ searchParams }: Props) {
  const q = await searchParams;
  const token = (q.token ?? "").trim();
  const errorMessage = loginQueryErrorMessage(q.error);
  const db = hasDatabase();
  const tokenOk = db && token ? await validatePasswordResetToken(token) : false;
      ...
      <section className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {!db ? (
          ...
          <p className="mt-2 text-sm text-slate-600">
            ...
            <Link href="/employer/forgot-password" className="font-medium text-blue-600 hover:text-blue-800">
              request a new one
            </Link>
            .
          </p>
        ...
        {db && token && tokenOk ? (
          <form action={resetEmployerPasswordAction} className="mt-6 space-y-4">
