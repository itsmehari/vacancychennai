...
- **Job detail SEO:** Per-job metadata uses `jobDetailPageMetadata()` in `src/lib/seo.ts`. Structured data: `buildJobPostingJsonLd` + `buildJobBreadcrumbListJsonLd` in `src/lib/job-posting-jsonld.ts` (`src/app/jobs/[jobId]/page.tsx`). Hyperlocal internal links: `listRelatedPublishedJobs` in `repository.ts`.
