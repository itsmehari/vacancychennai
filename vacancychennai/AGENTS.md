...
- **Sitemap / robots:** `src/app/sitemap.ts` merges static routes + area URLs (`listLocations`) + **`/blog/{slug}`** + **`/jobs.xml`** + **`/jobs/{id}`** for published roles (`listPublishedJobs`, up to 50k URLs per Google’s single-file limit). `robots.ts` exposes sitemap.
- **RSS:** Blog feed: `GET /rss.xml` (`src/app/rss.xml/route.ts`). Job listings feed: `GET /jobs.xml` (`src/app/jobs.xml/route.ts`) — newest postings for crawlers/aggregators. Root `layout.tsx` blog alternate via metadata `alternates.types`.
- **Job detail SEO:** Per-job metadata uses `jobDetailPageMetadata()` in `src/lib/seo.ts` (OG `type: "article"` + published/modified times when both provided). Visible intro: `buildFactualJobIntro` in `src/lib/job-seo-intro.ts`. O*NET hint for JSON-LD: `resolveOnetOccupationalCategoryUrl` in `src/lib/job-occupational-category.ts`. Structured data: `buildJobPostingJsonLd` + `buildJobBreadcrumbListJsonLd` in `src/lib/job-posting-jsonld.ts` (`src/app/jobs/[jobId]/page.tsx`). Domain `Job` includes `updatedAt` (Postgres `jobs.updated_at`). Hyperlocal internal links: `listRelatedPublishedJobs` in `repository.ts`.
- **Analytics:** GA4 via `GoogleAnalytics` in `src/components/google-analytics.tsx` (root layout); optional override `NEXT_PUBLIC_GA_MEASUREMENT_ID`. Partner outbound taps `src/components/partner-outbound-analytics.tsx`.
